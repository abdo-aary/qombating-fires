...
from qutils import get_distances_traps, chamfer_kernel, compute_intra_mmd_expectation, precompute_string_kernel, \
    compute_cross_mmd_expectation, mmd_kernel
from rydberg import RydbergEvolver
...
class BassirKernel(Kernel):
    ...
    def __init__(self, traps: Graph, positioner: Positioner, evolver: RydbergEvolver, **kwargs):
        """
        ...
        self.traps = traps
        self.distances = Register(traps).distances
        ...
        binary_tensor = self.evolver.binary_tensor  # shape (2^(n_qubits), n_qubits)
        self.string_kernel_mat = precompute_string_kernel(binary_tensor)
    ...
    def forward(self, x1: Tensor, x2: Tensor, **params) -> Union[Tensor, LinearOperator]:
        ...
        Then, using a precomputed string kernel Gram matrix (self.string_kernels) computed over the global
        ...
        psi_out1 = self.evolver(mask1)  # shape (..., N, 2^(n_qubits))
        psi_out2 = self.evolver(mask2)  # shape (..., M, 2^(n_qubits))
        global_probs1 = psi_out1.abs() ** 2  # shape (..., N, 2^(n_qubits))
        global_probs2 = psi_out2.abs() ** 2  # shape (..., M, 2^(n_qubits))

        string_kernel_mat = self.string_kernels.to(mask1.device)
        # For each sample, compute the intra-sample string kernel expectation.
        # Here, self.string_kernels is the precomputed Gram matrix K_B of shape (2^(n_qubits), 2^(n_qubits)).
